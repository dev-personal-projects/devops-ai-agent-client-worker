...
export default function PullRequestPage() {
  ...
  const autoMergeService = new GitHubAutoMergeService();
  ...
  const fetchPullRequest = async () => {
    try {
      ...
      if (response.error || !response.data) {
        throw new Error(
          response.error?.detail || "Failed to fetch pull request"
        );
      }

      setPullRequest(response.data);
      setCommitMessage(
        `Auto-merge PR #${response.data.number}: ${response.data.title}`
      );
    } catch (err) {
  ...
  const handleAutoMerge = async (method: MergeMethod) => {
    ...
    try {
      ...
      const response = await autoMergeService.autoMergePullRequest(pullNumber, {
        repository_url: pullRequest.base.repo.html_url,
        merge_method: method,
